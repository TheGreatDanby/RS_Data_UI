...
export default function JobDetailsCard({ ticket }) {
  ...
  //   useEffect(() => {
  //     fetchTicketDocuments(ticket.id, "invoices")
  //       .then((invoices) => {
  //         setRelatedDocuments((prev) => ({ ...prev, invoices }));
  //       })
  //       .catch((error) => console.error(error));

  //     fetchTicketDocuments(ticket.id, "estimates")
  //       .then((estimates) => {
  //         setRelatedDocuments((prev) => ({ ...prev, estimates }));
  //       })
  //       .catch((error) => console.error(error));
  //   }, [ticket.id]);
  useEffect(() => {
    const fetchDocuments = async () => {
      try {
        const invoices = await fetchTicketDocuments(ticket.id, "invoices");
        const estimates = await fetchTicketDocuments(ticket.id, "estimates");
        setRelatedDocuments({ invoices, estimates });
      } catch (error) {
        console.error(error);
      }
    };
        ...
        <div className="flex flex-col gap-4">
          ...
          {/* <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="font-medium">
                {relatedDocuments.invoices.number}
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="font-medium">
                {" "}
                {relatedDocuments.estimates.number}
              </div>
            </div>
          </div> */}
          <div className="flex flex-col gap-4">
              ...
              <div>
                ...
                {relatedDocuments.invoices.length > 0 ? (
                  <div className="flex flex-wrap space-x-4">
                    {relatedDocuments.invoices.map((invoice, index) => (
                      <div
                        key={index}
                        className="font-medium text-sky-500 hover:text-sky-700 cursor-pointer"
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                {relatedDocuments.estimates.length > 0 ? (
                  relatedDocuments.estimates.map((estimate, index) => (
                    <div
                      key={index}
                      className="font-medium text-sky-500 hover:text-sky-700 cursor-pointer"
